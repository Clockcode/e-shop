...
export const query = graphql`
  query($slug: String!) {
    strapiFooterCompanyPages(slug: { eq: $slug }) {
      Title
      PageContent
      slug
    }
...
const CompanyPages = ({ data }) => {
  console.info(data, "ozan")
  return (
    <React.Fragment>
      <Layout>
        <Header
        ...
        />
        <h1>{data.strapiFooterCompanyPages.Title}</h1>
        <p>{data.strapiFooterCompanyPages.PageContent}</p>
        <Footer
